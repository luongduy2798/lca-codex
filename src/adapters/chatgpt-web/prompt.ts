...
export type ChatGptContextTransport = "inline" | "mcp-pull";
...
export const CHATGPT_CONTEXT_CHUNK_TARGET_CHARS = 12_000;
...
function splitContextSnapshot(serialized: string): string[] {
  const chunks: string[] = [];
  let offset = 0;
  while (offset < serialized.length) {
    let end = Math.min(serialized.length, offset + CHATGPT_CONTEXT_CHUNK_TARGET_CHARS);
    if (end < serialized.length) {
      const previous = serialized.charCodeAt(end - 1);
      if (previous >= 0xD800 && previous <= 0xDBFF) end -= 1;
    }
    if (end <= offset) end = Math.min(serialized.length, offset + CHATGPT_CONTEXT_CHUNK_TARGET_CHARS + 1);
...
/**
 * Freeze the exact effective Codex context for one browser turn. The broker owns this immutable
 * snapshot for the lifetime of the turn; ChatGPT reads it through ordered MCP calls instead of
 * forcing the complete serialized history through the visible composer.
 */
export function compileChatGptContextSnapshot(parsed: CodexParsedRequest): ChatGptContextSnapshot {
  ...
  return {
    ...
    digest,
    serialized,
    chunks: splitContextSnapshot(serialized),
...
export function compileChatGptWebPrompt(
  ...
  const mcpPull = mode.localTools && !parsed._compactionRequest;
  const sharedContract = [
    "Act as the model backend for the Codex task referenced below.",
    mcpPull
      ? "The Codex task context is stored as an immutable broker snapshot and must be loaded through lca-token before you act."
    ...
    mcpPull
      ? "Load every Codex context chunk in strict cursor order and do not answer, reason about the task, or call any non-context tool until codex_context_next returns eof=true."
  ...
  const transportContract = parsed._compactionRequest
    ...
    : mode.localTools
    ? [
      "For local files, commands, processes, images, user interaction, and configured MCP/apps, use the attached lca-token plugin inside this same response.",
      ...
      "After binding, call codex_context_manifest, then repeatedly call codex_context_next with exactly the next_cursor returned by the previous context call until eof=true. These context calls are transport-only and never become outer Codex tool calls.",
      "Do not skip, reorder, summarize, or selectively fetch context chunks. The broker rejects lca-token execution until the final chunk has been delivered.",
      "turn_token and binding_id are different values: copy the exact binding_ value returned by codex_bind_turn into every later lca-token call, and never put the turn_ value in a binding_id field. Do not reveal either capability value in the answer.",
      "A bind result with binding_status active and valid_until outer_turn_end has no time limit. Never report that it expired unless a real lca-token call returns that exact error.",
      ...
      "lca-token synchronously bridges each plugin action into the same outer Codex turn; wait for its real result before continuing.",
      "Never serialize a proposed tool call as assistant text. Make the actual MCP call and use its real result.",
    ]
    : [
      `This is ChatGPT Web ${mode.displayLabel} with no lca-token bridge to the user's local computer attached to this response. This restriction applies only to local Codex files, commands, processes, and computer mutations.`,
  ...
  const transportResume = parsed._compactionRequest
    ...
    ? [
      "<codex_transport_resume>",
      `Your first action now must be codex_bind_turn with turn_token ${turnToken}. Then load the complete immutable Codex context through codex_context_manifest and codex_context_next before doing anything else.`,
      "Only after codex_context_next returns eof=true may you execute the latest active user request or call another lca-token tool.",
      "</codex_transport_resume>",
    ]
    : [
      "<codex_transport_resume>",
  ...
  const contextTransport = mcpPull
    ? [
      "<codex_context_ref>",
      JSON.stringify({
        version: 1,
  ...
  return {
    ...
    transport: mcpPull ? "mcp-pull" : "inline",
    ...(mcpPull ? {
      contextSnapshotId: snapshot.id,
      contextInputTokens: snapshot.estimatedTextTokens,
    } : {}),
  };
}
