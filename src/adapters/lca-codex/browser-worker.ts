...
export async function resolveChatGptToolConfirmation(
  ...
  while (Date.now() < deadline) {
    if (signal?.aborted) throw new DOMException("LCA Codex turn aborted", "AbortError");
...
export interface BrowserTurn {
  ...
  prepare: () => Promise<CompiledLcaCodexPrompt & { release: () => void }>;
  ...
  /** Append-only Markdown deltas emitted only at stable block boundaries. */
...
type ChatGptNetworkRecord = Record<string, unknown>;

const chatGptNetworkRecord = (value: unknown): ChatGptNetworkRecord | undefined => (
  value !== null && typeof value === "object" && !Array.isArray(value)
    ? value as ChatGptNetworkRecord
    : undefined
);

const chatGptNetworkString = (value: unknown): string | undefined => (
 ...
 *
 * The public DOM can disappear or remount while React is rendering a turn. The page-scoped
 * conversation request establishes submission and conversation ownership, while ChatGPT's
 * websocket exposes lifecycle events. Their arrival order is not a state machine, so evidence is
 * buffered and correlated by conversation/turn identity instead of by arrival order.
 */
export class ChatGptNetworkTurnTracker {
  private armed = false;
  private submissionKnown = false;
  ...
  observePageRequest(method: string, requestUrl: string): void {
    ...
    this.turnKnown = false;
    this.ownedConversationCreated = this.createdConversationIds.has(conversationId);

    const turnIds = this.streamTurnIds.get(conversationId);
    if (turnIds?.size === 1) {
      this.ownedTurnId = turnIds.values().next().value;
      this.turnKnown = true;
    }
  ...
  observeWebSocketPayload(payloadData: string): void {
    ...
    for (const value of entries) {
      const entry = chatGptNetworkRecord(value);
      if (!entry || entry.type !== "message") continue;
      const topicId = chatGptNetworkString(entry.topic_id);
      const envelope = chatGptNetworkRecord(entry.payload);
      const eventType = chatGptNetworkString(envelope?.type);
      const payload = chatGptNetworkRecord(envelope?.payload);
      if (!topicId || !eventType || !payload) continue;
      ...
      if (topicId.startsWith("conversation-turn-") && eventType === "conversation-turn-stream") {
        ...
        let turnIds = this.streamTurnIds.get(conversationId);
        if (!turnIds) {
        ...
        if (!this.ownedTurnId) continue;
        this.turnKnown = true;
        this.emitTransition("streaming");
        if (!this.ownedConversationCreated && this.createdConversationIds.has(conversationId)) {
          this.ownedConversationCreated = true;
          this.emitTransition("created");
        }
        this.tryCompleteOwnedTurn();
      ...
      if (topicId === "conversations" && eventType === "conversation-turn-complete") {
        ...
        const turnId = chatGptNetworkString(payload.turn_id);
        if (turnId) {
          let turnIds = this.completedTurnIds.get(conversationId);
          if (!turnIds) {
            turnIds = new Set<string>();
            this.completedTurnIds.set(conversationId, turnIds);
          }
          turnIds.add(turnId);
        } else {
  ...
  private tryCompleteOwnedTurn(): void {
    if (this.completed || !this.ownedConversationId) return;
    if (!this.ownedTurnId) {
      const completedTurnIds = this.completedTurnIds.get(this.ownedConversationId);
      if (completedTurnIds?.size === 1) {
        this.ownedTurnId = completedTurnIds.values().next().value;
        this.turnKnown = true;
      }
    }
    const exactTurnCompleted = this.ownedTurnId !== undefined
  ...
  snapshot(): ChatGptNetworkTurnState {
    return {
      armed: this.armed,
      submissionKnown: this.submissionKnown,
...
class ChatGptNetworkTurnObserver {
  ...
  async attach(page: Page): Promise<void> {
    ...
    session.on("Network.requestWillBeSent", event => {
      const request = chatGptNetworkRecord(chatGptNetworkRecord(event)?.request);
      const method = chatGptNetworkString(request?.method);
      const url = chatGptNetworkString(request?.url);
      if (method && url) this.tracker.observePageRequest(method, url);
    });
    try {
...
class ChatGptBrowserDiagnostics {
  ...
  async capture(page: Page, checkpoint: string, error?: unknown): Promise<{
    ...
    if (error === undefined
      && checkpoint !== "completion-pending-30s"
      && process.env.LCA_CODEX_BROWSER_DIAGNOSTICS !== "1") return;
    try {
      if (!this.initialized) {
      ...
      const [screenshotResult, stateResult] = await Promise.allSettled([
        ...
        page.evaluate(({
          ...
          return {
            ...
            surfaceId: (globalThis as typeof globalThis & { __LCA_CODEX_SURFACE_ID__?: unknown })
              .__LCA_CODEX_SURFACE_ID__ ?? null,
...
export function chatGptImageFilePayloads(images: LcaCodexPromptImage[]): Array<{ name: string; mimeType: string; buffer: Buffer }> {
  if (images.length > CHATGPT_MAX_INPUT_IMAGES) {
    throw new Error(`LCA Codex accepts at most ${CHATGPT_MAX_INPUT_IMAGES} input images per Codex turn`);
  }
  let totalBytes = 0;
  return images.map(image => {
    const parsed = parseDataUrl(image.imageUrl);
    if (!parsed) throw new Error(`LCA Codex input image ${image.ref} must be an inline base64 data URL`);
    const extension = imageExtensions.get(parsed.mediaType.toLowerCase());
    if (!extension) throw new Error(`LCA Codex input image ${image.ref} has unsupported media type: ${parsed.mediaType}`);
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(parsed.base64) || parsed.base64.length % 4 !== 0) {
      throw new Error(`LCA Codex input image ${image.ref} contains invalid base64 data`);
    }
    const buffer = Buffer.from(parsed.base64, "base64");
    if (buffer.length === 0) throw new Error(`LCA Codex input image ${image.ref} is empty`);
    if (buffer.length > 20_000_000) throw new Error(`LCA Codex input image ${image.ref} exceeds 20 MB`);
    totalBytes += buffer.length;
    if (totalBytes > 50_000_000) throw new Error("LCA Codex input images exceed the 50 MB per-turn limit");
    return { name: `${image.ref}.${extension}`, mimeType: parsed.mediaType.toLowerCase(), buffer };
  });
}

export function chatGptPromptFilePayloads(
  prompt: CompiledLcaCodexPrompt,
...
export class ChatGptBrowserWorker {
  ...
  private async runStage<T>(
    ...
    if (turnAbortSignal?.aborted) throw new DOMException("LCA Codex turn aborted", "AbortError");
    const startedAt = performance.now();
    console.info(`[lca-codex] browser turn ${traceId} stage=${stage} started`);
    ...
    try {
      ...
      const turnAbort = turnAbortSignal
        ? new Promise<never>((_, rejectAbort) => {
            turnAbortListener = () => {
              rejectAbort(new DOMException("LCA Codex turn aborted", "AbortError"));
              controller.abort();
            };
      ...
      console.info(`[lca-codex] browser turn ${traceId} stage=${stage} completed durationMs=${Math.round(performance.now() - startedAt)}`);
      return value;
    } catch (error) {
      console.error(`[lca-codex] browser turn ${traceId} stage=${stage} failed durationMs=${Math.round(performance.now() - startedAt)}: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    } finally {
  ...
  ): Promise<LcaCodexModelMode> {
    const mode = resolveLcaCodexModelMode(modelId, reasoning, capabilities);
  ...
  private async pendingCompletionDiagnostic(page: Page, responseTurn: Locator): Promise<string> {
  ...
  private async runBrowserTurn(turn: BrowserTurn, launcherSurfaceId?: string): Promise<string> {
    ...
    try {
      ...
      try {
        ...
        // DOM lifecycle heuristics when network instrumentation is unavailable.
        ...
        logLcaCodexActivity("lca_codex.network_observer_unavailable", {
          ...
          attempt,
          elapsedMs: activityDuration(startedAt),
          status: "initial_attach_failed",
          reason: error instanceof Error ? error.name : "Error",
        }, "warning");
        throw new Error(
          `ChatGPT network lifecycle observer is unavailable before Send: ${error instanceof Error ? error.message : String(error)}`,
          { cause: error },
        );
      }
      ...
      for (;;) {
        ...
        if (deadline !== undefined && Date.now() >= deadline) {
          throw new Error("LCA Codex turn timed out");
        }
        ...
        if (snapshot.responsePresent) {
          ...
          if (!loggedCompletionWait && Date.now() - sentAt >= 30_000) {
            ...
            const diagnostic = await this.pendingCompletionDiagnostic(page, responseTurn).catch(error => JSON.stringify({
              diagnosticError: error instanceof Error ? error.message : String(error),
            }));
            console.warn(
              `[lca-codex] waiting for network turn completion (textChars=${snapshot.visibleText.length}, networkConversationKnown=${networkState.conversationKnown}, networkTurnKnown=${networkState.turnKnown}, networkCompleted=${networkState.completed}, ui=${diagnostic})`,
            );
          }
        }
        if (networkCompletionReady || recoveredCompletionReady) {
          ...
          if (!final.markdown) {
            throw new Error(completedVisibleText
              ? "ChatGPT completed with visible text that could not be serialized as Markdown"
              : "ChatGPT completed without a renderable final answer");
          }
