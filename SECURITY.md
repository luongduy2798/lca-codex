...
Read the complete [security model](docs/security-model.md) before enabling the tool bridge. In
particular, tool-capable bridge turns let an untrusted model response request tools from the current
Codex turn; keep connector action control, Codex sandboxing, and approvals aligned with the
workspace's risk.
...
adapter to patched 2.0.12. `bun run audit`, the MCP protocol test, and the compiled-binary smoke test are
