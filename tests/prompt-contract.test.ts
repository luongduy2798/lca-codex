...
import { compileChatGptContextSnapshot, compileChatGptWebPrompt } from "../src/adapters/chatgpt-web/prompt";
...
test("tool-capable prompts bootstrap immutable MCP context pull before the task can run", () => {
  ...
  const resume = compiled.text.indexOf("<codex_transport_resume>", contextRefEnd);
  ...
  expect(contextRefEnd).toBeGreaterThan(0);
  expect(resume).toBeGreaterThan(contextRefEnd);
  expect(finalToken).toBeGreaterThan(resume);
  expect(compiled.text).toContain("codex_context_manifest");
  expect(compiled.text).toContain("codex_context_next");
  expect(compiled.text).toContain(snapshot.id);
  expect(compiled.text).not.toContain("<codex_context_json>");
  expect(compiled.text).not.toContain("preserve-system");
...
test("a long task keeps the newest images and drops the overflow instead of failing", () => {
  ...
  expect(compiled.text).not.toContain("step 13");
...
test("the replayed context never carries a finished turn's broker handles", () => {
  ...
  expect(compiled.text).not.toContain("keep working");
...
test("uses the public Instant name without leaking the browser menu alias into the prompt", () => {
  ...
  expect(compiled.text).toContain("This is ChatGPT Web Instant with no lca-token bridge to the user's local computer");
  expect(compiled.text).not.toContain("Instant 5.5");
});

test("keeps large tool-capable contexts intact in immutable MCP chunks instead of composer text", () => {
  const token = "turn_12345678901234567890123456789012";
  ...
  expect(compiled.text).toContain(snapshot.digest);
  ...
  expect(snapshot.chunks.join("")).toBe(snapshot.serialized);
});
